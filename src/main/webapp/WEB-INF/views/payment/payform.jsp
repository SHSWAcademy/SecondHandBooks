        ...
        <div class="flex-1 space-y-6">
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <div class="p-6 grid grid-cols-3 gap-6 text-sm">
                    <div><label class="block text-gray-500 mb-1">이름</label><p class="font-semibold">${sessionScope.loginSess.member_nicknm}</p></div>
                    <div><label class="block text-gray-500 mb-1">이메일</label><p class="font-semibold">${sessionScope.loginSess.member_email}</p></div>
                    <div><label class="block text-gray-500 mb-1">연락처</label><p class="font-semibold">${sessionScope.loginSess.member_tel_no}</p></div>
                </div>
            </div>
...
<script>
    ...
    document.getElementById('agreeCheckbox').addEventListener('change', function() {
       ...
       <script>
           ...
           // 5. 타이머
           let remainingSeconds = Number("${remainingSeconds}") || 300;
    ...
    const tossPayments = TossPayments("test_ck_KNbdOvk5rka22P9eoqA43n07xlzm");
    document.getElementById('payBtn').addEventListener('click', function() {
        // 결제 전 폼 유효성 검사 등 추가 가능
        tossPayments.requestPayment("토스페이", {
            amount: Number("${trade.sale_price}") + Number("${trade.delivery_cost}"),
            orderId: "ORDER_" + "${trade.trade_seq}" + "_" + new Date().getTime(),
            orderName: "${trade.book_title}",
            customerName: "${sessionScope.loginSess.member_nicknm}",
            successUrl: window.location.origin + "/payments/success?trade_seq=${trade.trade_seq}",
            failUrl: window.location.origin + "/payments/fail?trade_seq=${trade.trade_seq}"
        });
    });

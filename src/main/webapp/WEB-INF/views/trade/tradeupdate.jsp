                ...
                <section>
                    ...
                    <div id="selectedBookPreview" class="mt-6 p-6 bg-gradient-to-r from-blue-50 to-white rounded-[2rem] border border-blue-100 flex items-start gap-6 relative animate-[fadeIn_0.4s_cubic-bezier(0.22, 1, 0.36, 1)] group">
                        ...
                        <div class="flex-1 py-1">
                            ...
                            <h3 id="previewTitle" class="text-xl font-black text-gray-900 leading-tight mb-2 tracking-tight">${trade.book_title}</h3>
                            <div class="space-y-1 mb-4">
                                <p class="text-sm font-bold text-gray-600 flex items-center gap-2">
                                    <span id="previewAuthor">${trade.book_author}</span>
                                </p>
                                <p id="previewPublisher" class="text-xs font-medium text-gray-400">${trade.book_publisher}</p>
                            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-10">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea id="sale_cont" name="sale_cont" required rows="8"
                                      ...
                                      maxlength="500"
                                      class="w-full px-6 py-5 bg-gray-50 border border-transparent rounded-3xl text-gray-900 font-medium focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all outline-none resize-none leading-relaxed">${trade.sale_cont}</textarea>
                        </div>
                    </div>
                </section>
    ...
    function displaySearchResults(books) {
        ...
        books.forEach(book => {
            ...
            <img src="\${bImg ? bImg : '/img/no-image.png'}"
                 alt="\${bTitle}"
                 class="w-12 h-16 object-cover rounded-lg shadow-sm bg-white shrink-0" />
            <div class="flex-1 min-w-0 flex flex-col justify-center">
                <p class="font-bold text-gray-900 text-sm truncate">\${bTitle}</p>
                <p class="text-xs text-gray-500 truncate mt-0.5">\${bAuthor}</p>
                <div class="flex justify-between items-end mt-1">
                     <p class="text-[10px] text-gray-400 font-mono truncate">\${bIsbn}</p>
                     <p class="text-xs text-blue-600 font-bold">\${bPrice ? bPrice.toLocaleString() + '원' : ''}</p>
                </div>
            </div>`;

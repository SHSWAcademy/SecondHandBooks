        ...
        <%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
            <%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions" %>
                ...
                <head>
                    ...
                    <link rel="stylesheet"
                        href="${pageContext.request.contextPath}/resources/css/bookclub/bookclub.css">
                    <link rel="stylesheet"
                ...
                <body>
                                        ...
                                        <c:forEach var="club" items="${bookclubList}">
                                            <article class="bookclub-card" data-club-seq="${club.book_club_seq}">
                                                ...
                                                <button type="button" class="btn-wish ${club.wished ? 'wished' : ''}"
                                                    ...
                                                    <svg class="wish-icon" width="18" height="18" viewBox="0 0 24 24"
                                                        fill="${club.wished ? 'currentColor' : 'none'}"
                                                        ...
                                                        <c:choose>
                                                            <c:when test="${not empty club.banner_img_url}">
                                                                <%-- S3 URL (http/https)는 그대로, 로컬 경로(/)는 contextPath 붙임
                                                                    ...
                                                                    <c:choose>
                                                                        <c:when
                                                                            test="${club.banner_img_url.startsWith('http://') or club.banner_img_url.startsWith('https://')}">
                                                                            <c:set var="bannerSrc"
                                                                                value="${club.banner_img_url}" />
                                                                        </c:when>
                                                                        <c:when
                                                                            test="${club.banner_img_url.startsWith('/')}">
                                                                            <c:set var="bannerSrc"
                                                                                value="${pageContext.request.contextPath}${club.banner_img_url}" />
                                                                        </c:when>
                                                                        <c:otherwise>
                                                                            <c:set var="bannerSrc"
                                                                                value="${club.banner_img_url}" />
                                                        ...
                                                        <div class="card-body-inner">
                                                            ...
                                                            <div class="card-footer">
                                                                <span class="card-schedule">
                                                                    <c:choose>
                                                                        <c:when
                                                                            test="${club.book_club_schedule == null}">
                                                                            일정 미정
                                                                        </c:when>
                                                                        <c:otherwise>
                                                                ...
                                                                <span class="card-members">
                                                                    <c:out value="${club.joined_member_count}" />/
                            ...
                            <form id="createBookClubForm" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="toggle-group">
                                        <button type="button" class="toggle-btn active" data-value="offline"
                                            id="offlineToggle">오프라인</button>
                                        <button type="button" class="toggle-btn" data-value="online"
                                            id="onlineToggle">온라인</button>
                                    </div>
                                    ...
                                    <div class="place-search-container" id="modalPlaceSearchContainer">
                                        <div class="place-search-input-wrap">
                                            <input type="text" id="modalPlaceSearchInput" class="form-input"
                                                placeholder="장소를 검색하세요 (예: 스타벅스 강남)" autocomplete="off">
                                            <button type="button" id="modalPlaceSearchBtn"
                                                class="btn-place-search">검색</button>
                                        </div>

                                        <!-- 검색 결과 리스트 -->
                                        <div class="place-search-results" id="modalPlaceSearchResults"
                                            style="display:none;">
                                        ...
                                        <div class="selected-place" id="modalSelectedPlace" style="display:none;">
                                            ...
                                            <button type="button" class="btn-remove-place" id="modalRemovePlaceBtn"
                                                title="장소 삭제">X</button>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <!-- 모임 소개 -->
                                <div class="form-group">
                                    <textarea name="book_club_desc" class="form-textarea" placeholder="모임 소개"
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="schedule-row">
                                        <div class="toggle-group schedule-cycle">
                                            <button type="button" class="toggle-btn cycle-btn"
                                                data-value="매일">매일</button>
                                            <button type="button" class="toggle-btn cycle-btn"
                                                data-value="매주">매주</button>
                                            <button type="button" class="toggle-btn cycle-btn"
                                                data-value="매월">매월</button>
                                        </div>
                                    </div>
                                    <!-- 주차 선택 (매월 선택시만 표시) -->
                    ...
                    <script type="text/javascript"
                        src="//dapi.kakao.com/v2/maps/sdk.js?appkey=${fn:escapeXml(kakaoJsKey)}&libraries=services"></script>
